...
├── docker-compose.yml            # app + postgres (optional: manual/local run without k8s)
├── Jenkinsfile                   # CI/CD pipeline definition
├── jenkins-docker/
│   ├── Dockerfile                # custom Jenkins image: JDK17 + Maven + Docker CLI + kubectl baked in
...
Docker Desktop → **Settings (gear icon)** → **Kubernetes** → check
✅ **Enable Kubernetes** → **Apply & Restart**. Wait for the green
"Kubernetes running" indicator at the bottom left of Docker Desktop.

Confirm it from your Mac's terminal:
```bash
kubectl config current-context
```
Should print `docker-desktop`. If you use multiple clusters/contexts normally,
...
**(Optional) Test the manifests manually before letting Jenkins do it**, so
you can debug them directly if something's off:
```bash
cd k8s
...
kubectl apply -f 06-app-service.yaml

kubectl -n demo-app get pods -w
```
Wait until both `postgres-...` and `demo-app-...` pods show `1/1 Running`,
then:
```bash
curl http://localhost:9090/actuator/health
```
Docker Desktop automatically maps a `LoadBalancer` Service's external IP to
`localhost`, so no port-forwarding is needed.

If this manual test works, Jenkins doing the same `kubectl apply` commands
in the pipeline will work too.
